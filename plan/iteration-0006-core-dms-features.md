...
- **Middleware Enhancement**: Ensure the authentication middleware securely validates the JWT and extracts the `sub` claim (User ID) from Oathkeeper.
- **Document & Folder Endpoints (Full CRUD & Versioning)**:
  ...
  - `POST /api/documents`: Create a new document metadata entry (v1) and automatically insert a write relation tuple in Keto making the extracted JWT `sub` the `owner`.
