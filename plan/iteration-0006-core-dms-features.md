...
- [x] **Public Download Endpoint**: Route khusus (e.g. `/api/public/documents/{token}`) yang mem-bypass JWT middleware tetapi memvalidasi token dari database.
...
- **Bug 3 (Public Link 401)**: Link publik mengembalikan 401 Unauthorized karena router `api/public` tak sengaja diletakkan di bawah JWT middleware.
